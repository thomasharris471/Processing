...
void displayForceText() {
  ...
  textSize(abs(32*(float)Math.sin(theta)));  
  displayText("mg sin(\u03B8)", -mass*gravity*Math.sin(theta)*forceScale, 0);
  displayArrow(-mass*gravity*Math.sin(theta)*forceScale,0);
//  tomTranslate(-mass*gravity*Math.sin(theta),0);
  textSize(abs(32*(float)Math.cos(theta)));  
  displayText("mg cos(\u03B8)", 0,mass*gravity*Math.cos(theta)*forceScale);
  displayArrow(0,mass*gravity*Math.cos(theta)*forceScale);

  pop();
}

void displayForceTranslated() {
  double forceScale = length/(mass*gravity); 
  stroke(purple);
  ...
  tomTranslate(0,length);
  displayArrow(0,mass*gravity*Math.cos(theta)*forceScale);
  tomTranslate(0,mass*gravity*Math.cos(theta)*forceScale);
  displayArrow(-mass*gravity*Math.sin(theta)*forceScale,0);
//  tomTranslate(-mass*gravity*Math.sin(theta),0);
