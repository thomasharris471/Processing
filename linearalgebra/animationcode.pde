...
void initialize() {
  ...
  vectorsandmatrix();
   ...
   eigenvectors = computeEigenvectors(transform.row1.x, transform.row1.y, transform.row2.x, transform.row2.y);
...
void updateVectors() {
  ...
  tMatrix mat = qr.interpolate(time);
  for (int i = 0; i < basevectors.size(); i++) {
